...
class Ion_auth_model extends CI_Model
{
   ...
   * @author Mathew
   **/
  public function register($password, $email, $additional_data = array(), $groups = array(), $phone)
  {
    /*$dados = array(// Testar o retorno dos dados vindo do controller
  ...
  public function login($identity, $password, $remember=FALSE)
  {
    ...
    $query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, photo_user')
                      ->where($this->identity_column, $identity)
                      ->limit(1)
                ->order_by('id', 'desc')
                      ->get($this->tables['users']);
